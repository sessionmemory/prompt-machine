...
def external_api_call(model, prompt):
    ...
    api_url = ""
    first_choice_content = ""  # Initialize to avoid scope issues
    ...
    if model in ["gpt-4", "gpt-4-turbo", "gpt-4o", "o1-preview", "o1-mini", "gpt-4o-mini"]:
        ...
        # Generate content with correct argument names
        response = genai.generate_text(
            prompt=prompt,
            max_output_tokens=google_max_tokens,  # Correct argument name
            temperature=google_temperature,
        )
        # Process the response
        first_choice_content = process_google_response(response)
    ...
    if response_processor:
        ...
        print(f"{RESPONSE_COLOR}{first_choice_content}{RESET_STYLE}")  # This ensures the response is printed to the terminal
