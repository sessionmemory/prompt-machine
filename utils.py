...
def multi_selection_input(prompt, items):
    while True:
        ...
        for idx, item in enumerate(items, start=1):
            print(f"{idx}. \033[35m{item}\033[0m")
...
def select_category(categories):
    ...
    print("Enter '\033[35m0\033[0m' to enter a custom prompt.")
    print("Enter 'exit' to stop the program.")

    while True:
        category_input = input("\033[97m→ Enter the number of the \033[1;38;5;208mcategory\033[0m you want to use: \033[0m").strip()
        ...
        try:
            ...
            if category_idx == -1:
                ...
                # Confirmation step
                if not confirm_selection(f"Confirm your category selection '\033[1;38;5;208m{selected_category}\033[0m'? (y/n): "):
...
def print_response_stats(response, response_time, char_count, word_count):
    header_color = "\033[1m\033[93m"  # Bold and light yellow for header
    reset_color = "\033[0m"
    ...
    print(f"\n{header_color}Response Time:{reset_color} {formatted_response_time}")
    print(f"{header_color}Character Count:{reset_color} {char_count}")
    print(f"{header_color}Word Count:{reset_color} {word_count}")
    ...
    print(f"{header_color}Character Rate:{reset_color} {character_rate:.2f} characters per second")
    print(f"{header_color}Word Rate:{reset_color} {word_rate:.2f} words per second\n")

def get_user_rating():
    while True:
        rating = input("\033[97m→ Rate the response on a scale of \033[35m1 - 5\033[0m (5 being the best): \033[0m").strip()
        try:
            rating = int(rating)
            if 1 <= rating <= 5:
                return rating
...
def process_excel_file(model_name, prompt, excel_path):
    ...
    for index, row in df.iterrows():
        ...
        try:
            _, response, _, _, _ = generate(model_name, f"\033[35m{prompt}\033[0m {content}", None)
